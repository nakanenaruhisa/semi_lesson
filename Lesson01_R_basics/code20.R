...
ten_kukan20 %>%
  ...
  tbl_summary(
    label = list(height ~ "身長", weight ~ "体重", sex_c ~ "性別"),
...
ggplot(
  data = ten_kukan20,
  aes(x = height, y = weight, colour = sex_c)
) +
  geom_point(size = 2) +
  scale_colour_tableau() +
...
ggplot(data = ten_kukan20) +
  ...
  geom_boxplot() +
  geom_signif(
    comparisons = list(c("男性", "女性")), # `sex_c`のカテゴリー名
    test = "t.test",
    na.rm = FALSE,
