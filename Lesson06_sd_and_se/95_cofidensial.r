...
theme_set(theme_gray(
  # ggplot2のデフォルトテーマを設定する
  base_family = if (interactive()) "HiraginoSans-W3" else "sans", # フォントをヒラギノに指定（日本語の文字化け防止）
...
results <- purrr::map_dfr(sizes, function(n) {
  # sizesの各要素（20, 100, 1000）を順番に処理する
...
sim_results <- purrr::map_dfr(seq(10, 500, by = 10), function(n) {
  # seq()で10から500まで10刻みの数列を生成し、順番に処理する
...
# 実際に計算して確認
cat("\n--- SE = SD / √n の確認 ---\n") # 見出しを表示
for (n in c(4, 9, 25, 100, 400)) {
  # 5つのnについて順番に繰り返す
  cat(sprintf(
    "n = %3d → SE = SD / %.0f （SDの%.0f分の1）\n",
    n,
    sqrt(n),
    sqrt(n)
  ))
...
# 課題3-2: tibble を作って、SD のエラーバー付き棒グラフを描こう
...
#   それぞれの平均・SD・SE を tibble にまとめて表示しよう
