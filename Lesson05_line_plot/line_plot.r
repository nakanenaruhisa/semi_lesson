...
ggplot(
  company_long,
  aes(x = year, y = amount, color = category, linetype = category)
) +
  geom_line(linewidth = 1) +
  geom_point(size = 3) +
...
ggplot(
  company_long,
  aes(x = year, y = amount, color = category, shape = category)
) +
  geom_line(linewidth = 1) +
  geom_point(size = 3) +
...
ggsave(
  "Lesson05_line_plot/line_plot_result.png",
  plot = final_plot,
  width = 8,
  height = 5,
  dpi = 300
